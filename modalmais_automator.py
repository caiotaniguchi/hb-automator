...
class ModalmaisAutomator(HomeBrokerAutomator):
    ...
    def _select_stock(self, tab, stock):
        ...
        self._input_text_in_element_by_id('txtBoxPapel-or1', stock).send_keys(Keys.TAB)
        sleep(2)
    ...
    def _assert_price_in_alert_box(self, alert_box_text, field_name, price):
        assert re.search(field_name + r': (\d+\.\d+)', alert_box_text).group(1) == "{0:.2f}".format(price)
    ...
    def _confirm_stop_order(
        ...
        assert re.search(r'Quantidade: (\d+)', alert_box_text).group(1) == str(quantity)
        self._assert_price_in_alert_box(alert_box_text, 'Disparo Loss', stop_loss_trigger)
        self._assert_price_in_alert_box(alert_box_text, 'Limite Loss', stop_loss_price)
        self._assert_price_in_alert_box(alert_box_text, 'Disparo Gain', stop_gain_trigger)
        self._assert_price_in_alert_box(alert_box_text, 'Limite Gain', stop_gain_price)
    ...
    def _confirm_buy_order(self, stock, max_price, max_price_total, quantity):
        ...
        assert stock in alert_box_text
        assert re.search(r'Quantidade: (\d+)', alert_box_text).group(1) == str(quantity)
        assert float(re.search(r'Preço: (\d+\.\d+)', alert_box_text).group(1)) <= max_price
        assert float(re.search(r'Valor: (\d+\.\d+)', alert_box_text).group(1)) <= max_price_total
        self.driver.find_element_by_id('mb-button-yes').click()
    ...
    def enter_broker(self):
        ...
        self.driver.find_element_by_xpath('//button[@href="/Bolsa/Home"]').click()
    ...
    def send_buy_order(self, stock, max_price, max_price_total):
        ...
        if cur_price > max_price:
            raise ValueError("O valor de negociação é maior do que o máximo estipulado")
        if cur_price * 100 > max_price_total:
            raise ValueError("O lote mínimo é mais caro do que o limite estipulado")
        ...
        self._input_text_in_element_by_id('txtBoxAssDigital-or1', self.signature)
    ...
    def send_stop_order(
        ...
        self._input_text_in_element_by_id('txtBoxValidade-or1', good_until.strftime('%d%m%Y'))
        self._input_text_in_element_by_id('txtBoxQtde-or1', str(quantity))
        prices_to_fill = [
